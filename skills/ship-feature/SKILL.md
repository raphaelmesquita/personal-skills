...
Use the two custom agent roles installed by the `personal-skills` profile package:

- `ship_feature_implementation_fallback`
- `ship_feature_independent_review_fallback`

The repository-root `install-profile.ps1` discovers and installs these roles under Codex's personal agent directory. If a required role is unavailable, report the setup blocker and stop that fallback path; do not substitute a generic subagent.

The TOML owns each role's model, reasoning effort, sandbox, standing behavior, and generic output contract. The spawn brief owns only task-local facts: source of truth, workspace, current state, scope, acceptance criteria or invariants, relevant files, verification evidence or commands, and task-specific output requirements.

## Non-Negotiable Fallbacks

Do not skip delegation fallback gates.

- If `$delegate-antigravity` implementation fails because of quota, model availability, timeout, empty output, missing handoff, malformed handoff, or another runtime error, run exactly one `ship_feature_implementation_fallback` custom agent with task-local context, the same acceptance criteria, and explicit permission to edit the source-controlled workspace.
...
- If Opus review and the Gemini Antigravity review fallback both fail to produce usable review evidence, run exactly one `ship_feature_independent_review_fallback` custom agent before treating independent review as unavailable.
...
7. If fallback review is also empty, invalid, or blocked, run one `ship_feature_independent_review_fallback` custom agent using task-local context, and record that independent Antigravity review was unavailable.
